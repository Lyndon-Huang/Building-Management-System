...
public class Floor implements FireDrill {
    ...
    /**
     * the maintenance Schedule of floor.
     */
     ...
     * @throws IllegalArgumentException   if area is less than
     *                                    Room.getMinArea()
     ...
     * <p>
     * The new dimensions must be greater than or equal to the
     * minimum width and length for all floors.
     * <p>
     * Additionally, the new dimensions of the floor must be such that the
     * new total area is greater than or equal to the current occupied area
     * of the floor. This ensures that all the rooms currently on the floor
    ...
    public void changeDimensions(double newWidth, double newLength)
        ...
        if (newLength < Floor.getMinLength() ||
                newWidth < Floor.getMinWidth()) {
            throw new IllegalArgumentException();
        }
        if (newLength * newWidth < this.occupiedArea()) {
     ...
     * <p>
     * Maintenance will be undertaken on rooms on the floor
     * in the given order, with maintenance wrapping back to the start
     * of the order once all the rooms in the order have been visited.
     * <p>
     * The given room order must not be null and must contain at
     * least one room. All rooms in the given order must be rooms
     * on this floor, i.e. elements of getRooms(). The given order
     * does not need to contain all rooms on the floor, and it can
     * contain duplicates of the same room. However, if the order
     * contains more than one room, it cannot contain the same room
     * twice or more consecutively, where the start of the list and
     * the end of the list are also considered consecutive.
     * <p>
     * If this floor already has a maintenance schedule, it should be
     * replaced with the newly created schedule. The room currently being
     * maintained according to the old schedule should have its maintenance
     * status set to false.
     *
     ...
     *                                  if a room in the order is not on this
     *                                  floor, or if a room appears twice or
     *                                  more consecutively (the start of the
     *                                  list and the end of the list are also
     *                                  considered consecutive) in an order
     *                                  with at least two rooms.
     */
    public void createMaintenanceSchedule(List<Room> roomOrder)
            throws IllegalArgumentException {
        if (roomOrder == null || roomOrder.size() < 1) {
            throw new IllegalArgumentException("The roomOrder is null");
        }
        if (!this.getRooms().containsAll(roomOrder)) {
            throw new IllegalArgumentException(
                    "one of room not in this floor");
        }
        if (roomOrder.size() != 1) {
            for (int i = 0; i < roomOrder.size(); i++) {
                if (i == roomOrder.size() - 1) {
                    if (roomOrder.get(i).equals(roomOrder.get(0))) {
                        throw new IllegalArgumentException(
                                "room appears twice or more consecutively");
                    }
                } else {
    ...
    /**
     * Returns true if and only if this floor is equal
     * to the other given floor.
     * <p>
     ...
     * number of rooms
     * rooms (in any order). Comparison should either directly
     * or indirectly make use of each room's equals() method.
     *
    ...
    public boolean equals(Object obj) {
        boolean widthAndLengthInRange = true;
        boolean roomCompare = false;
        if (this == obj) {
            return true;
        }
        ...
        if (Math.abs(this.width - floor.getWidth()) > 0.001 ||
            ...
            widthAndLengthInRange = false;
        }

        boolean basicCondition = (this.floorNumber == floor.floorNumber
                && widthAndLengthInRange
                && this.getRooms().size() == floor.getRooms().size());

        if (this.getRooms().isEmpty() && floor.getRooms().isEmpty()) {
            roomCompare = true;
        } else if (basicCondition) {
            for (Room thisRoom : this.getRooms()) {
                roomCompare = false;
                for (Room floorRoom : floor.getRooms()) {
                    if (thisRoom.equals(floorRoom)) {
                        roomCompare = true;
                        break;
                    }
                }
        ...
        return (basicCondition && roomCompare);
    }

    /**
     * Returns the hash code of this floor.
     * <p>
     * Two floors that are equal according to equals(Object)
    ...
    public int hashCode() {
        int roomHash = 0;
        for (Room room : this.rooms) {
            roomHash += Objects.hashCode(room);
        }
        return Objects.hash(this.floorNumber, this.length, this.width,
                this.rooms.size(), roomHash);
    }
     ...
     * width in square metres, 'length' is this floor's length in square
     * metres, 'numRooms' is the number of rooms on this floor, and
     * 'encodedRoomX' is the encoded representation of this floor's Xth
     * room, if it exists, where rooms are appended in insertion order
     * (i.e. the order that rooms were added to this floor by calling
     * Floor.addRoom()). See Room.encode().
     * If this floor has a maintenance schedule, then the first line
     * of the returned string should instead follow the format:
     * <p>
    ...
    public String encode() {
        StringBuilder roomEncode = new StringBuilder();
        for (Room room : this.rooms) {
            roomEncode.append(room.encode());
        }
        if (this.maintenanceSchedule == null) {
            return (String.format("%d:%.2f:%.2f:%d" +
                            System.lineSeparator() + "%s",
                    this.floorNumber, this.width, this.length,
                    this.rooms.size(), roomEncode));
        } else {
            return (String.format("%d:%.2f:%.2f:%d:%s" +
                            System.lineSeparator() + "%s",
                    this.floorNumber, this.width, this.length,
