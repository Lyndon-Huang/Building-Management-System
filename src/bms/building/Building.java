...
public class Building implements FireDrill {
     ...
     * Comparison should either directly
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = 0; i < this.floors.size(); i++) {
            if (!this.floors.get(i).equals(building.getFloors().get(i))) {
                floorEqual = false;
                break;
            }
        }
     ...
     * this building and all of its floors, rooms and sensors.
     *
     * @return hash code of this building
     */
    @Override
    public int hashCode() {
        return Objects.hash(this.name, this.floors.size(), this.floors);
    }

    /**
     * Returns the machine-readable string representation
     * of this building and all of its floors, rooms and sensors.
     *
    ...
    public String encode() {
        ...
        for (Floor floor : this.floors) {
            encodedFloor.append(floor.encode());
        }
        return String.format("%s" + System.lineSeparator()
                + "%d" + System.lineSeparator()
                + "%s", this.name, this.floors.size(), encodedFloor);
    }
     ...
     * all floors above need to have dimensions small enough
     * so that this reduced floor can still support them.
     * If the floor is decreasing in size, it is essential
     * that it will remain large enough to be able to
     ...
     *                                  (i.e. getFloorByNumber() is null),
     *                                  or if newWidth < Floor.getMinWidth(),
     *                                  or newLength < Floor.getMinLength()
     * @throws FloorTooSmallException   if the floor below is too small
     *                                  to support increased dimensions,
     *                                  if the floor above is too large to be
     *                                  supported by decreased dimensions,
     *                                  or if the total size of the current
     *                                  rooms could not be supported by
    ...
    public void renovateFloor(int floorNumber, double newWidth,
        ...
        if (newWidth < Floor.getMinWidth()
            ...
            throw new IllegalArgumentException(
                    "width or length less than min width or length");
        }
        if (this.getFloorByNumber(floorNumber - 1).calculateArea() <
                newLength * newWidth ||
                this.getFloorByNumber(floorNumber + 1).calculateArea()
                        > newLength * newWidth ||
                floor.occupiedArea() > newLength * newWidth) {
            throw new FloorTooSmallException("too small");
        }
