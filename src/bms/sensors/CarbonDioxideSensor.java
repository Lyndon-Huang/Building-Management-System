...
public class CarbonDioxideSensor extends TimedSensor
     ...
     * @param variationLimit  acceptable range above and below ideal
     *                        value in ppm
     * @throws IllegalArgumentException if idealValue &lt;= 0;
     *                                  or if variationLimit &lt;= 0; or if
     ...
     * percentage given by: the (absolute) difference between the
     * ideal CO2 value and the current sensor reading, all divided
     * by getVariationLimit().
     * <p>
     * When the absolute difference between the ideal and the current
     * values is equal to or more than the variation limit, the comfort
     * is equal to zero percent (0).
     * <p>
     * Floating point division should be used when performing the
     * calculation, however the resulting floating point number should
     * be rounded to the nearest integer before being returned.
     *
     * @return the current comfort level as an integer between 0 and 100
     */
    @Override
    public int getComfortLevel() {
        int conforlevel = Math.abs((this.idealValue -
                this.getCurrentReading()) / this.getVariationLimit());
        if (conforlevel > 1) {
            return 0;
        } else {
            return (1 - conforlevel) * 100;
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        return (super.equals(carbonDioxideSensor)
                && this.idealValue == carbonDioxideSensor.getIdealValue()
                && this.variationLimit == carbonDioxideSensor
                .getVariationLimit());
    }
     ...
     * <p>
     * Two CO2 sensors that are equal according to equals(Object)
     ...
     * of this carbon dioxide sensor.
     * <p>
     * The format of the string to return is:
     * <p>
     * CarbonDioxideSensor:sensorReading1,sensorReading2,...,
     * sensorReadingN:frequency:idealValue:varLimit
     * <p>
     ...
     * 'varLimit' is this sensor's variation limit.
    ...
    public String encode() {
        return (String.format("CarbonDioxideSensor:%s:%d:%d:%d",
                super.encode(), this.getUpdateFrequency(), this.idealValue,
                this.variationLimit));
    }
